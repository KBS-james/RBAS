...
"use strict"; // Enforce strict mode to guard against global scope leaks
...
// FIXED: Properly scoped globally shared WebSocket references
let ws_dbio;
let send_dbio_req = []; 
let dbio_op_cnt = 0;

for (let i = 0; i < 100; i++) meter[i] = "\x1b[1;30m|";
...
function load_meter(start, color) {
  let t2 = Date.now();
  let t1_str = "" + start;
  t1_str = t1_str.substring(10, 12);
  let t2_str = "" + t2;
  t2_str = t2_str.substring(10, 12);
  start = Number(t1_str);
  let end = Number(t2_str);
...
// FIXED: Passed payload properties explicitly to prevent multi-socket race states
function valid_ps_request(in_data, p_id) {
  try {
    if (p_id >= ps_total) return false;
    if (in_data.ps_chain == ps_ledger_array[p_id].ps_chain) return true;
    if (in_data.ps_chain == ps_result[p_id].ps_next_chain) return true;
...
wss.on("connection", function (webSocket) {
  webSocket.on("message", function (message) {
    try {
      let local_in_json = JSON.parse(message);
      let local_ps_id = local_in_json.ps_id;
      ps_link[local_ps_id] = "conn";
      ps_rx_cnt[local_ps_id]++;
      if (valid_ps_request(local_in_json, local_ps_id)) {
        ps_request[local_ps_id] = local_in_json;
        ps_request[local_ps_id].tick = tick;
      }
      webSocket.send(JSON.stringify(ps_result_sync[local_ps_id]));
      ps_tx_cnt[local_ps_id]++;
    } catch (e) { }
  });
...
async function connect_dbio() {
  ...
  // FIXED: Removed local `let` re-declaration that was shadowing the global variable
  dbio_ip = par.rs_ip_array[db_id] + dbio_port; 
...
async function connect_sync(rs_index) {
  ...
  ws_rs[rs_index].onmessage = function (event) {
    let rx_buf = JSON.parse(event.data);
    let idx = rx_buf.rs_id;
    rs_rx_cnt[idx]++;
    db_sync[idx] = rx_buf;
  };
...
function display() {
  let dt = new Date();
  let str = dt.toISOString().substring(11, 19);
  if (time != str) {
    console.clear();
    console.log("\x1b[1;33m|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|")
    time = str;
    let m_str = "";
    for (let i = 0; i < 100; i++) m_str = m_str + meter[i];
    console.log(m_str);
    for (let i = 0; i < 100; i++) meter[i] = "\x1b[1;37m|";
    ...
    for (let i = 0; i < 2; i++) {
      ...
      d_array[i].tick = db_sync[i] ? db_sync[i].tick : undefined;
      d_array[i].chain = db_sync[i] ? db_sync[i].chain : undefined;
      d_array[i].next_chain = db_sync[i] ? db_sync[i].next_chain : undefined;
    }
    ...
    for (let i = 0; i < 3; i++) {
      let ps_id_ = i;
      let ps_link_ = ps_link[i];
      let ps_rx_cnt_ = ps_rx_cnt[i];
      let ps_tx_cnt_ = ps_tx_cnt[i];
    ...
    d_array = [];
    for (let i = 0; i < 5; i++) { if (ps_request[i] != undefined) d_array[i] = ps_request[i]; }
    console.table(d_array);


    console.log("\x1b[1;32m ps_request[0].ts_request[0..] = \x1b[1;37m");
    d_array = [];
    for (let i = 0; i < 3; i++) { try { d_array[i] = ps_request[0].ts_request[i]; } catch (e) { d_array[i] = {} } }
    console.table(d_array);

    console.log("\n\x1b[1;33m|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|=========|")
    console.log("\x1b[1;31m ps_result[i] (wait for sync) \x1b[1;37m");
    d_array = [];
    for (let i = 0; i < 5; i++)  d_array[i] = ps_result[i];
    console.table(d_array);
    ...
    d_array = [];
    try { for (let i = 0; i < 5; i++) d_array[i] = ps_result_sync[i]; } catch (e) { d_array[i] = {} }
    console.table(d_array);
  ...
  let req0 = 0, req2 = 0, res0 = 0, res2 = 0;
  ...
  if (isNaN(t_index)) t_index = 0;
  if (t_index >= disp_row) t_index = t_index - 30;
  if (t_index >= 0 && t_index < disp_row) sys_disp[t_index] = {};
}
...
function save_files() {
  if (tick > db_summary.tick) {
    ...
    let dt = new Date();
    let str = dt.toISOString().substring(0, 11);
    let yy = Number(str.substring(0, 4));
    let mm = Number(str.substring(5, 7));
    let dd = Number(str.substring(8, 10));
    console.log("yy = " + yy + "; mm = " + mm + "; dd = " + dd);
    let log_filename = "log/" + yy + "/" + mm + "/" + "log_db_summary_" + dd + ".jsonl";
    
    // FIXED: Removed line referencing undeclared variable 'json' that caused silent crashes
    let strSummary = JSON.stringify(db_summary);
    let data_str = strSummary + "\n";
...
function gen_db_summary() {
  if (summary_tick != tick) {
    summary_tick = tick;
    let start_db_ptr = db_ptr;
    
    // FIXED: Enclosed inside brackets and changed to sequential string aggregation to prevent full hash chain collapse
    let pwd = chain;
    for (let i = 0; i < ps_total; i++) {
      pwd += JSON.stringify(ps_result[i]);
    }
    pwd = pwd + tick + JSON.stringify(db_ledger);
    
    hash = crypto.createHash("sha256").update(pwd).digest("hex");
    next_chain = hash.substring(0, 16);
    let ledger = db_ledger;
...
function rs_sync() {
  if (db_sync[db_id] && tick != db_sync[db_id].tick) {
    vote = 0;
    let test_tick = db_sync[db_id].tick;
    for (let i = 0; i < rs_total; i++) {
      if (db_sync[i] && db_sync[i].tick == test_tick) { vote++; }
    }
    if (vote == 9) {
...
function update_ledger() {
  for (let i = 0; i < ps_total; i++) {
    try {
      if (ps_result[i] && ps_request[i] && ps_result[i].ps_chain == ps_request[i].ps_chain) {
        if (ledger_valid(ps_result[i].ps_ledger)) {
          if (ps_ledger_array[i].ps_thread != ps_result[i].ps_thread) {
            ...
            let ps_ledger = ps_ledger_array[i];
            let ts_result = ps_result[i].ts_result;
...
// FIXED: Removed dangling trailing '}' brace that generated syntax compiling exceptions
